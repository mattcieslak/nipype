...
from ..dsi_studio import Reconstruction


def test_Reconstruction_inputs():
    input_map = dict(
        args=dict(argstr='%s', ),
        check_btable=dict(
    ...
    inputs = Reconstruction.input_spec()

    for key, metadata in list(input_map.items()):
        for metakey, value in list(metadata.items()):
            assert getattr(inputs.traits()[key], metakey) == value
def test_Reconstruction_outputs():
    output_map = dict(output_fib=dict(), )
    outputs = Reconstruction.output_spec()
