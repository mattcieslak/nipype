...
from ..dsi_studio import GQIReconstruction


def test_GQIReconstruction_inputs():
    input_map = dict(
        args=dict(argstr='%s', ),
        check_btable=dict(
        ...
        ratio_of_mean_diffusion_distance=dict(
            argstr='--param0=%.4f',
            usedefault=True,
        ),
    ...
    inputs = GQIReconstruction.input_spec()

    for key, metadata in list(input_map.items()):
        for metakey, value in list(metadata.items()):
            assert getattr(inputs.traits()[key], metakey) == value
def test_GQIReconstruction_outputs():
    output_map = dict(output_fib=dict(), )
    outputs = GQIReconstruction.output_spec()
