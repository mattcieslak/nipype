...
class CreateSrcInputSpec(CommandLineInputSpec):
    ...
    nifti_file = File(desc="DWI Nifti file", argstr="--source=%s",
            exists=True, copyfile=False)
    dicom_dir = File(desc="Directory with DICOM data from only the dwi",
            exists=True, argstr="--source=%s")
    bvals_file = File(desc="Text file containing b values",
            exists=True, argstr="--bval=%s")
    bvecs_file = File(desc="Text file containing b vectors (FSL format)",
            exists=True, argstr="--bvec=%s")
    b_table_file = File(desc="Text file containing q-space sampling (DSI Studio format)",
            exists=True, argstr="--b_table=%s")
    recursive = traits.Bool(False, desc="Search for DICOM files recursively",
            argstr="--recursive=1")
    output_src = File(name_template="%s.src.gz", desc="Output file (.src.gz)",
            argstr="--output=%s",name_source="subject_id", genfile=True)
    grad_dev = File(desc="Gradient deviation file", exists=True,
            copyfile=True, 
            position=-1, argstr="#%s")

class CreateSrcOutputSpec(TraitedSpec):
    src_file = File(name_template="%s.src.gz", desc="Output file (.src.gz)",
            argstr="--output=%s",name_source="subject_id" )

class CreateSrc(CommandLine):
    """ DSI Studio can read data from NIfTI (with bvals, bvecs or btable 
    text files) or a directory full of DICOM files. The src.gz file 
    contains the diffusion images and gradient information.
    
    Notes:
    ---------------
    This step will fail if you use a NIfTI input that contains negative
    values.
    
    If a HCP-style grad_dev.nii.gz file is available for NIfTI inputs,
    this file is linked into the working directory and detected by 
    dsi_studio. It is not passed as a commandline argument. 
    
    Examples:
    ---------
    >>> from nipype.interfaces import dsi_studio
    >>> make_src = dsi_studio.CreateSrc()
    >>> make_src.inputs.nifti_file = 'DWI.nii.gz'
    >>> make_src.inputs.bvals_file = 'DWI.bval'
    >>> make_src.inputs.bvecs_file = 'DWI.bvec'
    >>> make_src.inputs.grad_dev = 'grad_dev.nii.gz'
    >>> make_src.cmdline # doctest: +ELLIPSIS
    'dsi_studio --action=src '
    >>> make_src.run() # doctest: +SKIP 
...
class ReconstructionInputSpec(CommandLineInputSpec):
    input_src_file = File(desc="DSI Studio src file", mandatory=True, 
            exists=True, copyfile=False, argstr="--source=%s")
    mask = File(desc="Volume to mask brain voxels", exists=True,
            copyfile=False,argstr="--mask=%s")
    thread_count = traits.Int(1,usedefault=True,argstr="--thread_count=%d")
    output_odf = traits.Bool(False, usedefault=True,
            desc="Include full ODF's in output", argstr="--record_odf=1")
    odf_order = traits.Enum((8, 4, 5, 6, 10, 12, 16, 20), usedefault=True,
            desc="ODF tesselation order")
    check_btable = traits.Enum((1,0), usedefault=True, 
            argstr="--check_btable=%d",
            desc="Check if btable matches nifti orientation (not foolproof)")
    num_fibers = traits.Int(3,usedefault=True,argstr="--num_fiber=%d",
    ...
    decomposition = traits.Bool(False, argstr="--decomposition=1",
            desc="Apply ODF Decomposition")
    decomp_fraction = traits.Float(0.05, desc="Decomposition Fraction",
            argstr="--param3=%.2f", requires=["decomposition"])
    decomp_m_value = traits.Int(10, desc="Decomposition m value",
            argstr="--param4=%d",requires=["decomposition"])
    
    # Deconvolution traits
    deconvolution = traits.Bool(False, argstr="--deconvolution=1",
            desc="Apply ODF Deconvolution")
    deconv_regularization = traits.Float(0.5, requires=["deconvolution"],
            desc="Deconvolution regularization parameter", argstr="--param2=%.2f")

class ReconstructionOutputSpec(TraitedSpec):
    output_fib = File(desc="Output File",exists=True)

class GQIReconstructionInputSpec(ReconstructionInputSpec):
    ratio_of_mean_diffusion_distance = traits.Float(1.25, usedefault=True, 
            argstr="--param0=%.4f")

class DSIReconstructionInputSpec(ReconstructionInputSpec):
    hamming_window_len = traits.Int(16, argstr="--param0=%d")
    

class Reconstruction(CommandLine):
    input_spec = ReconstructionInputSpec
    output_spec = ReconstructionOutputSpec 
    cmd="dsi_studio --action=rec "

class DTIReconstruction(Reconstruction):
...
class GQIReconstruction(Reconstruction):
    cmd="dsi_studio --action=rec --method=4"
    input_spec = GQIReconstructionInputSpec
    ...
    def _list_outputs(self):
        ...
        target = os.path.join(os.getcwd(),srcname) + \
            ...
            self.inputs.num_fibers,self.inputs.ratio_of_mean_diffusion_distance)
        print("search target", target)
        results= glob(target)
        assert len(results) == 1
        outputs["output_fib"] = results[0]
...
class ExportInputSpec(CommandLineInputSpec):
    input_file = File(exists=True, argstr="--source=%s",mandatory=True,
            copyfile=False)
    to_export = traits.Str(mandatory=True,argstr="--export=%s")

class ExportOutputSpec(CommandLineInputSpec):
...
class Export(CommandLine):
    input_spec = ExportInputSpec
    output_spec = ExportOutputSpec
...
def compute_mda(fib_file):
    ...
    nib.Nifti1Image(output.reshape(shape,order="F")[::-1,::-1,:], 
...
mda_from_fib = Function(
        input_names=["fib_file"],
        output_names=["mda_file"],
        function=compute_mda)

class ConnectivityMatrixInputSpec(CommandLineInputSpec):
    tract_file = File(exists=True,argstr="--tract=%s")
    input_fib = File(exists=True, argstr="--source=%s", mandatory=True,
            copyfile=False)
    ...
    # ROI related options
    seed_file = File(exists=True,desc="specify the seeding file. " + \
        "Supported file format includes text, Analyze, and nifti files.",
        argstr="--seed=%s")
...
class ConnectivityMatrix(CommandLine):
    ...
    def _list_outputs(self):
        ...
        assert len(results) > 1
        outputs["connectivity_matrices"] = [os.path.abspath(c) for c in results]
        return outputs




class TrackingInputSpec(ConnectivityMatrixInputSpec):
    ...
    output_trk = traits.Str(name_template="%s.trk.gz", desc="Output file (trk.gz)",
            argstr="--output=%s",name_source="input_fib")#, genfile=True)
